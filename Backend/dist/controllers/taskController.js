...
class TaskController {
    ...
    static async getTasks(req, res) {
        try {
            ...
            // Check card exists
            const card = await firebase_1.FirebaseService.getCardById(cardId);
            if (!card || card.boardId !== boardId) {
                return res.status(404).json({ error: 'Card not found' });
            }
            const tasks = await firebase_1.FirebaseService.getTasksByCardId(cardId);
    ...
    static async createTask(req, res) {
        try {
            ...
            const task = await firebase_1.FirebaseService.createTask({
                cardId,
                boardId,
                ownerId: userId,
                title,
    ...
    static async updateTask(req, res) {
        try {
            ...
            if (card_id && card_id !== cardId) {
                const targetCard = await firebase_1.FirebaseService.getCardById(card_id);
                if (targetCard && targetCard.boardId === boardId) {
                    updates.cardId = card_id;
                }
            }
            await firebase_1.FirebaseService.updateTask(taskId, updates);
            const updatedTask = await firebase_1.FirebaseService.getTaskById(taskId);
            res.status(200).json({
                id: updatedTask.id,
    ...
    static async deleteTask(req, res) {
        try {
            ...
            for (const attachment of attachments) {
                await firebase_1.FirebaseService.deleteGitHubAttachment(attachment.id);
            }
            await firebase_1.FirebaseService.deleteTask(taskId);
            res.status(204).send();
        }
        catch (error) {
    ...
    static async assignMember(req, res) {
        try {
            ...
            if (!task.assignedMembers.includes(memberId)) {
                ...
                await firebase_1.FirebaseService.updateTask(taskId, { assignedMembers: updatedAssignedMembers });
            }
            res.status(201).json({
                taskId: taskId,
                memberId: memberId
    ...
    static async getAssignedMembers(req, res) {
        try {
            const { boardId, cardId, taskId } = req.params;
            const userId = req.user.id;
            // Check board access
