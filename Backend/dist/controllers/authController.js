...
class AuthController {
    static async sendCode(req, res) {
        try {
            ...
            const existingUser = await firebase_1.FirebaseService.getUserByEmail(email);
            ...
            await firebase_1.FirebaseService.createVerificationCode({
                email,
                code,
                type: type,
                expiresAt,
            ...
            // Send email
            await (0, email_1.sendVerificationCode)(email, code, type);
            res.status(200).json({
    ...
    static async signup(req, res) {
        try {
            ...
            // Verify code
            const validCode = await firebase_1.FirebaseService.getValidVerificationCode(email, verificationCode, 'signup');
            if (!validCode) {
            ...
            // Create user
            const user = await firebase_1.FirebaseService.createUser({ email });
            // Mark verification code as used
            await firebase_1.FirebaseService.markVerificationCodeAsUsed(validCode.id);
            // Generate token
    ...
    static async signin(req, res) {
        try {
            ...
            // Verify code
            const validCode = await firebase_1.FirebaseService.getValidVerificationCode(email, verificationCode, 'signin');
            if (!validCode) {
                return res.status(400).json({ error: 'Invalid or expired verification code' });
            }
            // Get user
            const user = await firebase_1.FirebaseService.getUserByEmail(email);
            if (!user) {
